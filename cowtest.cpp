...
//    glBegin(GL_POINTS);
//
//    glVertex2d(double(12*2) / WINDOW_WIDTH, double(-(cnt++)*2)/WINDOW_HEIGHT );
//
//    glEnd();
//}
...
void draw() {
    ...
    for (int i=0; i<faces.size(); i++)  {
        glNormal3f(faces[i].lines[0], faces[i].lines[1], faces[i].lines[2]);
        glVertex3f(faces[i].lines[0], faces[i].lines[1], faces[i].lines[2]);
    }
    glEnd();
    glFlush();
    glutSwapBuffers();
